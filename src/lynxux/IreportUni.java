 ...
 * @author rajika
 */
public class IreportUni {
    Connection conn;
    PreparedStatement pst;
    ResultSet rst;
    public IreportUni(String sql,String button) {
    ...
    private void build1(String sql) {
		...
		try {
			report()//create new report design
			  ...
			  .title(cmp.text("Your Leaves Report").setStyle(boldCenteredStyle))//shows report title
			...
			report()//create new report design
			  ...
			  .columns(//add columns
			  	...
			  	col.column("Status", "status", type.stringType()),
                                col.column("Reason", "reason", type.stringType()),
                                col.column("Date", "date", type.stringType()))                               
			  .title(cmp.text("Additional Leaves Report").setStyle(boldCenteredStyle))//shows report title
	...
	private void build3(String sql) {
		...
		try {
			report()//create new report design
			  ...
			  .title(cmp.text("Employee Report").setStyle(boldCenteredStyle))//shows report title
	...
	private JRDataSource createDataSource1(String sql) {
            ...
            String Type;
            String Status;
            DRDataSource dataSource = new DRDataSource("date", "type", "reason");
            try{
